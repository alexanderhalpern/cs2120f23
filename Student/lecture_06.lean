...
open Object -- load the Object type so we can use rock, paper, scissors directly
#check rock
...
-- eval vs reduce?
-- when you define your own type then use Reduce

-- value of a product type is 
...
-- looks like applying function Box.put to "Hello" but it is just evaluated and sits there

#check Box.put
#check (@Box.put) -- STUDY THIS RESULTS
