...
-- Problem 1
-- Every dog likes some cat.
variable (Dog Cat : Type) (Likes : Dog → Cat → Prop)
#check ∀ (d : Dog), ∃ (c : Cat), Likes d c

-- Problem 2
-- If any dog, d, likes any dog, g, and that dog, g, likes any dog, w, then d likes w.
variable (Dog Cat : Type) (Likes : Dog → Dog → Prop)
#check ∀ (d g w : Dog), Likes d g → Likes g w → Likes d w

-- Problem 3
-- If any cat, c, likes any cat, d, then d also likes c.
variable (Dog Cat : Type) (Likes : Cat → Cat → Prop)
...
variable (Dog Cat : Type) (Likes : Cat → Cat → Prop)
#check ∀ (c d : Cat), (∀ (x : Cat), Likes c x ∧ Likes d x) → Likes c d


-- Formal Proof of Problem 5
example : ∀ (c d : Cat), (∀ (x : Cat), Likes c x ∧ Likes d x) → Likes c d :=
  fun (c d : Cat) (h : ∀ (x : Cat), Likes c x ∧ Likes d x) => (h d).left
